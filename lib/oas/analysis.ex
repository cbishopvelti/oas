...
defmodule Oas.Analysis do
  ...
  def outstanding_tokens(from, to) do
    _fromDT =
      Timex.to_naive_datetime(Date.add(from, 1))
      |> DateTime.from_naive!("Etc/UTC")

    toDT =
      Timex.to_naive_datetime(Date.add(to, 1))
      |> DateTime.from_naive!("Etc/UTC")
    ...
    tokens =
      from(t in Oas.Tokens.Token,
        ...
        where:
          ((not is_nil(tr.id) and tr.when <= ^to) or
             (is_nil(tr.id) and t.inserted_at < ^toDT)) and
            t.expires_on > ^from and (t.used_on > ^from or is_nil(t.used_on)),
        select: t,
        order_by: [desc: t.id]
      )
      |> Oas.Repo.all()
  ...
  def outstanding_attendance(from, to) do
    ...
    attendance =
      from(att in Oas.Trainings.Attendance,
        ...
        where:
          tra.when <= ^to and
            (is_nil(tok.used_on) or tok.used_on >= ^from) and
            is_nil(cre.id)
    ...
    frequency =
      ...
      |> Enum.reduce(%{}, fn attendance, acc ->
        ...
        endDate =
          case attendance.token && attendance.token.used_on do
            nil -> to
            x -> Date.add(x, -1)
          end

        value =
          case attendance.token do
            %{value: value} -> value
            _value -> minValue
          end
