...
defmodule Oas.Credits.Credit do
      ...
      case args do
        %{amount: amount} ->
          ...
          |> changeset(%{
            ...
            expires_on:
              if(amount > 0.0,
                do: get_field(changeset, :when) |> Date.add(config.token_expiry_days),
                else: nil
              )
          })
      end
  ...
  def get_global_credits() do
    members =
      from(m in Oas.Members.Member, where: true)
      |> Oas.Repo.all()
  ...
  defp deduct_debit(
         [%{amount: %Decimal{sign: sign}} = head | tail],
         %{amount: %Decimal{sign: sign}} = debit,
         opts
       ) do
    ...
    out =
      cond do
        ...
        Map.get(head, :expires_on, nil) != nil and Date.after?(debit.when, head.expires_on) ->
          # assume rest of the credits are expired as they are ordered
          [head | tail] ++ [debit]
  ...
  defp process_credits(credits) do
    {trans, active_ledger} =
      credits
      |> Enum.map_reduce([], fn credit, ledger ->
        # IO.puts("vvvvvvvvvvvvvvvv")
        new_ledger = deduct_debit(ledger, credit)

        after_amount =
          Enum.reduce(new_ledger, Decimal.new("0.0"), fn %{amount: amount}, acc ->
    ...
    {
      trans,
      Enum.reduce(active_ledger, Decimal.new("0"), fn %{amount: amount}, acc ->
        Decimal.add(acc, amount)
      end)
    }
  end
  ...
  def get_credits(member, now \\ Date.utc_today()) do
    ...
    non_expired_subquery =
      from(c in Oas.Credits.Credit,
        where:
          (is_nil(c.expires_on) or (not is_nil(c.expires_on) and ^now <= c.expires_on)) and
            c.who_member_id == ^member.id,
        select: {c, c.amount}
      )
    ...
    expired_subquery =
      from(c in Oas.Credits.Credit,
        ...
        where:
          not is_nil(c.expires_on) and ^now > c.expires_on and
            c.who_member_id == ^member.id,
        group_by: c.id,
