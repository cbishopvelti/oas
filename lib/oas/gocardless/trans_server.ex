  ...
  def handle_info(:init, state) do
    ...
    case Oas.Gocardless.Transactions.process_transacitons() do
      ...
      {:to_many_requests, headers} ->
        remaining =
          List.keyfind!(headers, "http_x_ratelimit_account_success_remaining", 0)
          |> elem(1)
          |> String.to_integer()
        ...
        reset_seconds =
          ...
          |> elem(1)
          |> String.to_integer()
          # Insure we have a little buffer.
          |> Kernel.+(60)
      ...
      {:unauth_401, error} ->
        ...
        {:stop, :normal, state |> Map.put(:timer_ref, nil)}
    end
  end
