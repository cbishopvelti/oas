...
defmodule Oas.Gocardless.Transactions do
  ...
  defp process_tokens(out_transaction) when is_map_key(out_transaction, :state) do
    out_transaction
  end
      ...
      configToken = %{quantity: quantity, value: value} ->
        if is_map_key(out_transaction, :who_member_id) && out_transaction.who_member_id != nil do
          ...
          Map.put(
            ...
            :warnings,
            [
              "This looks like tokens, but related member (via bank_account_name) was not found"
              | Map.get(out_transaction, :warnings, []) || []
            ]
          )
        end
    end
  ...
  defp process_membership(out_transaction) when is_map_key(out_transaction, :state) do
    out_transaction
  end
  ...
  defp process_membership(out_transaction) do
    ...
    membershipPeriod =
      Oas.Members.MembershipPeriod.getThisOrNextMembershipPeriod(
          ...
          Decimal.eq?(amount, value) ->
            if is_map_key(out_transaction, :who_member_id) && out_transaction.who_member_id != nil do
              ...
              Map.put(out_transaction, :warnings, [
                "This looks like a membership, but related member (via gocardless_name: \"" <>
                  out_transaction.who <> "\") was not found"
                | Map.get(out_transaction, :warnings, []) || []
              ])
            end
  ...
  defp generate_transaction(
    ...
    auto_tags =
      Oas.Transactions.TransactionTagAuto.get_auto_tags(
        out_transaction.who,
        out_transaction.who_member_id
      )
    ...
    |> Ecto.Changeset.put_assoc(
      :gocardless_transaction_iid,
      %Oas.Transactions.Gocardless{}
      |> Oas.Transactions.Gocardless.changeset(%{
        gocardless_data:
          JSON.encode!(in_transaction |> Map.drop([:name, :maybe_member, :date, :amount])),
        warnings:
          case Map.get(out_transaction, :warnings, nil) do
            nil -> nil
            [] -> nil
    ...
    |> Oas.Transactions.TransactionTags.doTransactionTags(%{
      transaction_tags:
        ((Map.get(out_transaction, :tags, []) |> Enum.map(fn name -> %{name: name} end)) ++
           auto_tags)
  ...
  def process_transactions_2(transactions) do
    ...
    |> Enum.map(fn transaction ->
      name =
        Map.get(transaction, "debtorName") || Map.get(transaction, "creditorName") ||
          Map.get(transaction, "remittanceInformationUnstructured")
          |> String.slice(0..26)
          |> String.trim()
    ...
    |> Enum.map(fn transaction ->
      ...
      if !config.credits do
        ...
        Oas.Gocardless.TransactionsCredits.generate_transaction_credits(transaction)
      end
    end)
  end
  ...
  def get_transactions_real(from \\ nil, to \\ nil) do
    ...
    with {:ok, 200, headers, client} <-
           :hackney.request(
             :get,
             "https://bankaccountdata.gocardless.com/api/v2/accounts/#{account_id}/transactions/?#{Enum.join(query, "&")}",
      ...
      {:ok, 429, headers, _client} ->
        seconds_retry =
          List.keyfind!(headers, "http_x_ratelimit_account_success_reset", 0)
          |> elem(1)
          |> String.to_integer()

        Logger.warning(
          "Gocardless.Transactions.process_transacitons(#{from}) failed, retrying in #{seconds_retry}s"
        )
