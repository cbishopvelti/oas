...
defmodule Oas.Gocardless.TransactionsCredits do
  ...
  defp process_credits_2(changeset, data) do
    ...
    configToken =
      case Oas.Attendance.get_token_amount(%{
             member_id: Ecto.Changeset.get_field(changeset, :who_member_id)
           }) do
        x when x < 0 ->
          configTokens =
          ...
          max_num =
            Decimal.div(amount, configToken.value)
      ...
      nil ->
        changeset =
          ...
          |> Ecto.Changeset.put_assoc(:credit, %{
            ...
            amount: Ecto.Changeset.get_field(changeset, :amount),
            expires_on:
              Date.add(changeset |> Ecto.Changeset.get_field(:when), config.token_expiry_days)
          })
  ...
  def generate_transaction_credits_2(changeset, data) do
    {out_changeset, data} =
      case Ecto.Changeset.get_field(changeset, :who_member_id) do
    ...
    out_changeset =
      ...
      |> Oas.Transactions.TransactionTags.doTransactionTags(%{
        transaction_tags:
          Map.get(data, :transaction_tags, []) |> Enum.map(fn tag -> %{name: tag} end)
      })
