  ...
  def processDuplicates(rows) do
    ...
    |> Enum.map(
      fn row = %{
        ...
        who =
          case Map.get(row, :who_member_id, nil) do
        ...
        case dupliate do
          ...
          %{id: id} ->
            row
            |> Map.put(:errors, [
              %{name: :duplicate, transaction_id: id} | Map.get(row, :errors, [])
            ])
            ...
            cond do
              Decimal.eq?(amount, value) ->
                if is_map_key(row, :who_member_id) do
                  ...
                  Map.put(row, :warnings, [
                    "This looks like a membership, but related member (via bank_account_name: \"" <>
                      row.bank_account_name <> "\") was not found"
                    | Map.get(row, :warnings, [])
                  ])
                end
    ...
    |> Enum.map(fn
      ...
      row ->
        if is_map_key(row, :who_member_id) do
          ...
          configToken =
            case Oas.Attendance.get_token_amount(%{member_id: Map.get(row, :who_member_id)}) do
              x when x < 0 ->
                configToken =
          ...
          Map.put(row, :warnings, [
            "Didn't find member (via bank_account_name), so not adding credits"
            | Map.get(row, :warnings, [])
          ])
        end
    end)
        ...
        if Map.get(row, :state, nil) == :credits || Map.get(row, :state, nil) == :tokens_credits do
          doCredits(
            %{
              ...
              value:
                amount -
                  (Map.get(row, :state_data, %{}) |> Map.get(:value, 0) |> Decimal.to_float()) *
                    (Map.get(row, :state_data, %{}) |> Map.get(:quantity, 0))
            },
            result
          )
        end
