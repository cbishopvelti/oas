...
defmodule Oas.Llm.Room do
  ...
  def init(init_args) do
    ...
    messages =
      LangChain.Utils.to_serializable_map(
        struct(LangChain.Chains.LLMChain, %{
          messages: [init_message]
        }),
        [:messages]
    ...
    {:ok, chat} =
      case result do
        # New chat
        {:ok, chat} ->
          # Manually mark members as loaded and empty.
          {:ok, %{chat | members: [], seen: []}}

        # Constraint error, already exists
        {:error, %{errors: [topic: _]}} ->
          chat =
  ...
  def handle_info(%{event: "presence_diff", payload: %{joins: joins}}, state) do
    ...
    new_members =
      joins
      |> Enum.flat_map(fn {_k, join} ->
        join.metas
        # filter out anonymous members
        |> Enum.filter(fn
    ...
    chat =
      if new_members |> length > 0 do
        new_chat =
          state.chat
          |> Ecto.Changeset.change()
          # Only add members if they're not admins.
          |> Ecto.Changeset.put_assoc(
            :members,
            ((new_members
              |> Enum.filter(fn %{is_admin: is_admin} ->
                if length(state.chat.members) == 0 do
                  true
                else
                  !is_admin
                end
              end)) ++ state.chat.members)
            |> Enum.uniq_by(fn %{id: id} -> id end)
          )
          |> Oas.Repo.update!(returning: true)

        # Update seen
        (new_members ++ state.chat.seen)
        |> Enum.uniq_by(fn %{id: id} -> id end)
        |> Enum.map(fn member ->
        ...
        # If first, start llm client for first person.
        if OasWeb.Channels.LlmChannelPresence.list(state.topic) |> Map.to_list() |> length == 1 &&
             OasWeb.Channels.LlmChannelPresence.list(state.topic)
             |> Map.to_list()
             |> List.first()
             |> elem(0) == joins |> Map.to_list() |> List.first() |> elem(0) &&
             config.llm_enabled do
          meta =
            OasWeb.Channels.LlmChannelPresence.list(state.topic)
            |> Map.to_list()
            |> List.first()
  ...
  def handle_info(%{event: "message", payload: message}, state) do
    ...
    # Ensure the messaging member is added to the members
    new_chat =
      if !is_nil(member) do
        new_member =
          struct(Oas.Members.Member, member |> Map.put(:name, message.metadata.presence_name))
