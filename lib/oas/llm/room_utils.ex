...
defmodule Oas.Llm.RoomUtils do
  def notify_someone(message, state) do
    ...
    {members_to_notify, send_admins} =
      case members_to_notify do
    ...
    presence =
      ...
      |> Enum.flat_map(fn {_id, %{metas: metas}} ->
        Enum.map(metas, fn
          %{member: %{id: member_id}, presence_id: presence_id} -> {member_id, presence_id}
          %{presence_id: presence_id} -> {nil, presence_id}
        end)
      end)
      |> MapSet.new()

    # Mark as seen, as they're in the room currently
    presence
    |> Enum.filter(fn
      {nil, _} -> false
      _ -> true
    ...
    presence_x_sender =
      presence
      |> MapSet.reject(fn {_member_id, presence_id} ->
        presence_id == message.metadata |> get_in([:presence_id])
      end)

    members_to_notify_ids =
      members_to_notify
