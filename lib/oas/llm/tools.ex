        ...
        function: fn params, context ->
          case context.member |> Map.get(:id) do
            nil ->
              {:error,
               "Unauthorized, login " <>
                 "[here](#{Phoenix.VerifiedRoutes.unverified_url(OasWeb.Endpoint, OasWeb.Router.Helpers.member_session_path(OasWeb.Endpoint, :new))})"}
            ...
            member_id ->
              case from(t in Oas.Trainings.Training,
                   ...
                   |> Oas.Repo.one() do
                nil ->
                  {:error, "Error finding event/traning/jam to add you to."}

                # Oas.Attendance.add_attendance(%{member_id: 1, training_id: 267},%{inserted_by_member_id: 1})
                event ->
                  try do
                    {:ok, _attendance} =
                    ...
                    Absinthe.Subscription.publish(OasWeb.Endpoint, %{success: true},
                      user_attendance_attendance: member_id
                    )

                    Absinthe.Subscription.publish(OasWeb.Endpoint, %{id: event.id},
                      attendance_attendance: event.id
                    )
                    ...
                    {:error, [%{message: "training_id: has already been taken"} | _]} ->
                      {:error,
                       "The user is already booked in to the jam/training. Tell the user they're already booked in."}
  ...
  def get_upcoming_events_tool do
    Function.new!(%{
      ...
      function: fn _, context ->
        ...
        {:ok,
         "These are are in addition to and override the events you already know about: \n" <> out}
      end
    })
  end
