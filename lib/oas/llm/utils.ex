defmodule Oas.Llm.Utils do
  ...
  def decode(item, _opts) when is_map(item) do
    Enum.map(item, fn {k, v} ->
      ...
      case key do
        ...
        :tool_calls ->
          {key, decode(v, struct: ToolCall)}

        :tool_results ->
          {key, decode(v, struct: ToolResult)}

        :status ->
          {key, decode(v) |> String.to_atom()}
  ...
  def get_presence_name(socket) do
    case socket do
      %{assigns: %{current_member: current_member}} ->
        current_member.name

      %{assigns: %{llm: llm, from_channel_context: %{member: member}}} ->
        "#{llm.name} for #{member.name}"

      %{assigns: %{llm: llm}} ->
        "#{llm.name} for anonymous"
