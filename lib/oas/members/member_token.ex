defmodule Oas.Members.MemberToken do
  use Ecto.Schema
  import Ecto.Query
  alias Oas.Members.MemberToken
  ...
  def build_session_token(member) do
    ...
    {token, %MemberToken{token: token, context: "session", member_id: member.id}}
  end
  ...
  def member_and_contexts_query(member, :all) do
    from t in MemberToken, where: t.member_id == ^member.id
  end

  def member_and_contexts_query(member, [_ | _] = contexts) do
    from t in MemberToken, where: t.member_id == ^member.id and t.context in ^contexts
  end
end
