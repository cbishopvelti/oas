...
defmodule Oas.TokenMailer do
  ...
  def maybe_send_warnings_email(member) do
    ...
    case attendance do
      [_firstAttendance, %{token: nil, training: %{when: when1}}]
      when when1 > lastTransaction.when ->
        nil
  ...
  def should_send_warning(last_transaction, credits) do
    last_debits =
      credits
      |> Enum.filter(fn %{amount: amount, when: when1} ->
  ...
  def maybe_booking_warning(_member, attendance) do
    case from(a in Oas.Trainings.Attendance,
           ...
           where:
             a.id == ^attendance.id and a.member_id != a.inserted_by_member_id and
  ...
  def warning_email(member, %{attendance: attendance} \\ %{attendance: nil}) do
    ...
    items =
      maybe_send_credits_warning(member) ++
        case attendance do
          nil ->
            []
