  ...
  def do_auto_tags(old_who, who, transaction_tags, auto_tags) do
    ...
    if old_who !== who do
      case from(tr in Oas.Transactions.Transaction,
           ...
           |> Oas.Repo.exists?() do
        # Old still exist, delete relevent tags
        true ->
          from(ta in Oas.Transactions.TransactionTagAuto,
            where:
              (ta.who == ^old_who or ta.who == ^who) and
                ta.transaction_tag_id not in ^(auto_tags |> Enum.map(fn %{id: id} -> id end)) and
                ta.transaction_tag_id in ^(transaction_tags |> Enum.map(fn %{id: id} -> id end))
          )
          |> Oas.Repo.delete_all()

        # No old who remaining, so delete all transaciton tags from old
        false ->
          from(ta in Oas.Transactions.TransactionTagAuto,
    ...
    Oas.Repo.insert_all(
      Oas.Transactions.TransactionTagAuto,
      auto_tags
