...
defmodule OasWeb.Channels.LlmChannel do
  ...
  def handle_info(:after_join, socket) do
    ...
    # Presence
    {:ok, _} =
      OasWeb.Channels.LlmChannelPresence.track(
        socket,
        Oas.Llm.Utils.get_presence_id(socket),
        metas
      )
    ...
    push(socket, "messages", %{
      ...
      who_am_i:
        ...
        |> (&if(!is_nil(socket_to_member_map(socket)),
              do: Map.put(&1, :member, socket_to_member_map(socket)),
              else: &1
            )).()
    })
  ...
  defp debounce_data({:delta, message}, socket) do
    new_delta =
      if socket.assigns |> Map.has_key?(:delta) &&
           !(socket.assigns |> Map.get(:delta) |> is_nil()) do
    ...
    if message.status == :complete do
      ...
      if is_nil(socket.assigns |> Map.get(:delta_debounce, nil)) ||
           !(socket.assigns |> Map.get(:delta_debounce, nil) |> Process.read_timer()) do
        push(socket, Atom.to_string(:delta), new_delta |> delta_to_out())

        Phoenix.Socket.assign(socket, :delta_debounce, Process.send_after(self(), :delta, 20))
        |> assign(:delta, nil)
      else
  ...
  def handle_in("toggle_llm", %{"presence_id" => presence_id, "value" => value}, socket)
      when is_bitstring(presence_id) do
    case (socket.assigns |> Map.get(:current_member, %{is_admin: false})).is_admin ||
           presence_id == socket.assigns |> Map.get(:presence_id) do
      # is authed to do this
      true ->
        case value do
          true ->
            channel_meta =
              OasWeb.Channels.LlmChannelPresence.get_by_key(socket.topic, presence_id).metas
              |> List.first()
          ...
          false ->
            my_pids =
              OasWeb.Channels.LlmChannelPresence.list(socket)[presence_id]
              |> Map.get(:metas)
