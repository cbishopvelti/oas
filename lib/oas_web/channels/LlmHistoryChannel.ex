...
defmodule OasWeb.Channels.LlmHistoryChannel do
  ...
  def handle_info(:after_join, socket) do
    history = case socket.assigns do
      %{current_member: %{is_admin: true}} ->
        ...
        history = from(c in Oas.Llm.Chat,
          preload: [members: ^members],
          where: true,
          order_by: [desc: :updated_at],
          limit: 1256 # DEBUG ONLY, set higher
        ) |> Oas.Repo.all()
  ...
  def handle_out("new_history", new_history_item, socket) do
    ...
    case !is_nil(socket.assigns.current_member.is_admin) and socket.assigns.current_member.is_admin do
      true ->
        ...
        old_history_index = socket.assigns.history
        |> Enum.find_index(fn (%{id: id}) ->
          id == new_history_item.id
        end)

        new_history = if (old_history_index != nil) do
      ...
      false ->
        if new_history_item.members |> Enum.any?(fn (%{id: id}) -> id == socket.assigns.current_member.id end) do
          push(socket, "new_history", new_history_item)
  ...
  def handle_out("llm_presence_diff", %{joins: joins, leaves: leaves}, socket) do
    ...
    push(socket, "presence_diff", %{
      joins: joins |> Map.filter(fn {_k, v} ->
        MapSet.member?(topics, v.topic)
      end),
      leaves: leaves |> Map.filter(fn {_k, v} ->
        MapSet.member?(topics, v.topic)
      end)
    })
