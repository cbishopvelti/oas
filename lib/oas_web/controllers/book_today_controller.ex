...
defmodule OasWeb.BookTodayController do
  ...
  def get_one_training(trainings, now) do
    distances = Enum.flat_map(trainings, fn (%{start_time: start_time, end_time: end_time} = training) ->
      [
        (if !is_nil(start_time), do: {DateTime.diff(now, start_time) |> abs(), training}, else: nil),
    ...
    case distances do
      [] -> {:tomany, nil}
      distances ->
        candidates = Enum.group_by(distances, fn  ({dist, _training}) ->
          dist
        end)
        |> Enum.to_list()
        ...
        case candidates do
          ...
          {_dist, dist_trainings} ->
            ...
            |> Enum.filter(fn %{start_time: start_time, end_time: end_time} ->
              DateTime.compare(start_time, now) in [:lt, :eq] and DateTime.compare(end_time, now) in [:gt, :eq]
            end)
            case candidates do
              [one] -> {:ok, one}
              _ -> {:tomany, nil}
  ...
  def index(conn, _params) do
    ...
    trainings = from(t in Oas.Trainings.Training,
      where: t.when == ^now
    ) |> Oas.Repo.all()
    ...
    case {status, training} do
      {:tomany, _} ->
        conn
        |> put_flash(:error, "There is more than one event today. Please select your desired event from the booking page linked below.")
        |> render("index.html", %{
