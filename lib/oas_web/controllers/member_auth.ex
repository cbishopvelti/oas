defmodule OasWeb.MemberAuth do
  import Plug.Conn
  ...
  def log_out_member(conn) do
    ...
    member_token && Members.delete_session_token(member_token)

    if live_socket_id = get_session(conn, :live_socket_id) do
      OasWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  def fetch_current_member(conn, _opts) do
    ...
    member = member_token && Members.get_member_by_session_token(member_token)
  ...
  defp signed_in_path(conn) do
    ...
    case Plug.Conn.fetch_cookies(conn, signed: OasWeb.CallbackPathPlug.callback_path_cookie())
         |> Map.get(:cookies)
         |> Map.get(OasWeb.CallbackPathPlug.callback_path_cookie()) do
      nil ->
        "/"
      ...
      %{
        ...
        [
          external:
            Application.get_env(:oas, callback_domain |> String.to_atom()) <> callback_path
        ]
    end
  end
end
