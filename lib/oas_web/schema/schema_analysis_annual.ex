      ...
      resolve(fn parent, _, _ ->
        tokens =
          from(att in Oas.Trainings.Attendance,
            ...
            where:
              tra.when <= ^parent.to and
                is_nil(tok.used_on) and
                is_nil(cre.id),
            select: count(att.id)
          )
          |> Oas.Repo.one!()
      ...
      resolve(fn parent, _b, _c ->
        ...
        tagged =
          ...
          |> Enum.reduce(%{}, fn tr, acc ->
            my_tags =
              tr.transaction_tags |> Enum.map(fn %{name: name} -> name end) |> MapSet.new()
            ...
            case {tr.credit, tr.tokens} do
              ...
              {cr, [_x | _rest]} when not is_nil(cr) ->
                out = {
                  ...
                  tr.tokens
                  |> Enum.reduce(Decimal.new("0"), fn t, acc -> Decimal.add(t.value, acc) end)
                }
        ...
        from(tr in Oas.Transactions.Transaction,
          ...
          where:
            tr.when >= ^parent.from and tr.when <= ^parent.to and tr.amount >= ^0 and
              (is_nil(to.id) and is_nil(cr.id))
        )
        |> Oas.Repo.all()
      ...
      resolve(fn _, %{from: from, to: to} = args, _ ->
        ...
        {:ok,
         %{
           ...
           to: to,
           transaction_tags:
             MapSet.new(transaction_tags |> Enum.map(fn %{name: name} -> name end))
         }}
      end)
    end
  end
