...
defmodule OasWeb.Schema.SchemaCredits do
  ...
  object :credits_mutations do
    ...
    field :transfer_credit, :success do
      ...
      resolve(fn _,
                 %{from_member_id: from_member_id, to_member_id: to_member_id, amount: amount},
                 _ ->
        ...
        {weighted_data, _, _} =
          ...
          |> Enum.reduce_while({[], Decimal.new(amount), 0}, fn credit,
                                                                {accData, amount, usedAmount} ->
            newAmount = Decimal.sub(amount, Decimal.sub(credit.after_amount, usedAmount))

            {cont, newAccData} =
              if Decimal.lt?(newAmount, "0.0") do
        ...
        # Average
        {sumprod, sum} =
          Enum.reduce(weighted_data, {0.0, 0.0}, fn {amount, date}, {sumprod, sum} ->
            {
