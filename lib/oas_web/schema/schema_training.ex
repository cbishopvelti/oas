      ...
      resolve(fn _,
        ...
        results =
          from(t in Oas.Trainings.Training,
            ...
            where:
              (0 == ^length(training_tag_ids) or tt.id in ^training_tag_ids) and
                t.when >= ^from and t.when <= ^to and
                (0 == ^length(training_where) or
                   w.id in ^(training_where |> Enum.map(fn %{id: id} -> id end)))
          )
          |> Oas.Repo.all()
  ...
  object :training_mutations do
    ...
    field :insert_training, type: :training do
      ...
      resolve(fn _, args, _ ->
        ...
        training_where =
          case training_where do
            %{id: id} ->
              Oas.Repo.get!(Oas.Trainings.TrainingWhere, id)

            data ->
              value =
                from(t in Oas.Config.Tokens,
                  select: max(t.value)
                )
                |> Oas.Repo.one()
    ...
    field :update_training, type: :training do
      ...
      resolve(fn _, args, _ ->
        ...
        training =
          Oas.Repo.get!(Oas.Trainings.Training, args.id)
          |> Oas.Repo.preload(:training_tags)
          |> Oas.Repo.preload(:training_where)
        ...
        removedTrainingTags =
          Ecto.Changeset.get_change(toSave, :training_tags, [])
          |> Enum.filter(fn
            %{action: :replace} -> true
            _ -> false
          end)
