...
defmodule OasWeb.Schema.SchemaTransaction do
  ...
  object :transaction_queries do
    field :transactions, list_of(:transaction) do
      ...
      resolve(fn _,
                 args = %{from: from, to: to, transaction_tags: transaction_tags},
                 %{context: _context} ->
        ...
        query =
          ...
          |> (&(case transaction_tag_ids do
                  ...
                  ids ->
                    join(&1, :inner, [t], tt in assoc(t, :transaction_tags), as: :tt)
        ...
        result =
          ...
          |> Enum.map(fn transaction ->
            warnings =
              (Map.get(transaction, :gocardless_transaction_iid, %{}) || %{})
              |> Map.get(:warnings)
    ...
    field :transaction, :transaction do
      ...
      resolve(fn _, %{id: id}, _ ->
        transaction =
          Oas.Repo.get!(Oas.Transactions.Transaction, id)
        ...
        warnings =
          (Map.get(transaction, :gocardless_transaction_iid, %{}) || %{}) |> Map.get(:warnings)
  ...
  defp maybeDoMembership(
    ...
    _membership =
      from(m in Oas.Members.Membership, where: m.transaction_id == ^id)
      |> Oas.Repo.one()
      |> case do
        nil ->
      ...
      resolve(fn _, %{id: id, who_member_id: who_member_id}, _ ->
        ...
        out_transaction =
          ...
          |> Oas.Gocardless.TransactionsCredits.generate_transaction_credits_2(%{
            transaction_tags:
              transaction.transaction_tags |> Enum.map(fn %{name: name} -> name end)
          })
