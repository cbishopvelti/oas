defmodule OasWeb.Schema.SchemaTransactionsImport do
  ...
  object :transactions_import_mutations do
    field :set_to_import, type: :success do
      ...
      resolve(fn _,
                 %{index: index, to_import: to_import},
    ...
    field :import_transactions, type: :success do
      ...
      resolve(fn _,
        ...
        result =
          ...
          |> CSV.decode(
            headers: true,
            field_transform: fn field -> String.trim(field) end,
          ...
          |> Enum.map(fn
            data = %{memo: memo} ->
              [bank_account_name, my_reference] =
                memo
                |> String.split("\t")
    ...
    field :do_import_transactions, type: :success do
      ...
      resolve(fn
        # %{to_import: to_import},
        _, _, %{context: %{user_table: user_table, current_member: current_member}} ->
          [{_, rows}] = :ets.lookup(user_table, current_member.id)
