      ...
      resolve(fn _, _, conn ->
        ...
        {:ok,
         %{
           ...
           membership_status:
             Oas.Attendance.check_membership(Map.get(context, :current_member, %{})) |> elem(1),
      ...
      resolve(fn _, _, %{context: %{current_member: %{id: id}}} ->
        ...
        result =
          from(trai in Oas.Trainings.Training,
            ...
            on: atte.training_id == trai.id and atte.member_id == ^id,
            left_join: memb in assoc(atte, :member),
            left_join: ac in subquery(attendance_counts),
            on: ac.training_id == trai.id,
            preload: [training_where: [:training_where_time], attendance: {atte, [member: memb]}],
            where:
              trai.when >= ^Date.utc_today() and
          ...
          |> Enum.map(fn {booking, limit} ->
            training_where_time =
              Oas.Trainings.TrainingWhereTime.find_training_where_time(
                booking,
                booking.training_where.training_where_time
              )
  ...
  object :user_mutations do
    field :user_add_attendance, :success do
      ...
      resolve(fn _,
                 ...
                 %{context: %{current_member: %{id: member_id}}} ->
    ...
    field :user_undo_attendance, :success do
      ...
      resolve(fn _,
        ...
        case config.enable_booking do
          true ->
            result =
              from(atte in Oas.Trainings.Attendance,
                join: trai in assoc(atte, :training),
                preload: [training: [training_where: [:training_where_time]]],
                where:
                  atte.id == ^attendance_id and
                    atte.inserted_by_member_id == ^member_id and atte.member_id == ^member_id and
                    (((is_nil(trai.commitment) or trai.commitment == false) and
                        trai.when >= ^Date.utc_today()) or
                       (trai.when == ^Date.utc_today() and
                          atte.inserted_at < ^DateTime.add(now, 60)))
              )
              |> Oas.Repo.one()
            ...
            case result do
              nil ->
                {:error, "Unable to undo"}

              %{
                id: id,
                inserted_at: inserted_at
              } ->
                if NaiveDateTime.compare(inserted_at, booking_cutoff) == :lt or
                     NaiveDateTime.compare(inserted_at, booking_cutoff) == :eq or
                     NaiveDateTime.compare(inserted_at, NaiveDateTime.shift(now, minute: 1)) ==
                       :lt or
                     NaiveDateTime.compare(inserted_at, NaiveDateTime.shift(now, minute: 1)) ==
                       :eq do
  ...
  object :user_subscriptions do
    field :user_attendance_attendance, :success do
      config(fn _args, context ->
        ...
        {:ok,
         topic: [
           context |> Map.get(:context) |> Map.get(:current_member) |> Map.get(:id),
           "global"
         ]}
      end)
