defmodule OasWeb.MemberAuthTest do
  use OasWeb.ConnCase
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, OasWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "log_in_member/3" do
    test "stores the member token in the session", %{conn: conn, member: member} do
      ...
      assert Members.get_member_by_session_token(token)
    end
  ...
  describe "logout_member/1" do
    test "erases session and cookies", %{conn: conn, member: member} do
      ...
      assert redirected_to(conn) == "/"
      refute Members.get_member_by_session_token(member_token)
    end

    test "broadcasts to the given live_socket_id", %{conn: conn} do
      live_socket_id = "members_sessions:abcdef-token"
      OasWeb.Endpoint.subscribe(live_socket_id)
