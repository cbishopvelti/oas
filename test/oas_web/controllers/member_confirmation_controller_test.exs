defmodule OasWeb.MemberConfirmationControllerTest do
  use OasWeb.ConnCase
  ...
  describe "POST /members/confirm" do
    ...
    test "sends a new confirmation token", %{conn: conn, member: member} do
      ...
      assert Repo.get_by!(Members.MemberToken, member_id: member.id).context == "confirm"
    end

    test "does not send confirmation token if Member is confirmed", %{conn: conn, member: member} do
      Repo.update!(Members.Member.confirm_changeset(member))
      ...
      assert redirected_to(conn) == "/"
      assert get_flash(conn, :info) =~ "If your email is in our system"
      refute Repo.get_by(Members.MemberToken, member_id: member.id)
    end
    ...
    test "does not send confirmation token if email is invalid", %{conn: conn} do
      ...
      assert redirected_to(conn) == "/"
      assert get_flash(conn, :info) =~ "If your email is in our system"
      assert Repo.all(Members.MemberToken) == []
    end
  end
  ...
  describe "POST /members/confirm/:token" do
    test "confirms the given token once", %{conn: conn, member: member} do
      ...
      assert Members.get_member!(member.id).confirmed_at
